...
const pkey = process.env.PEENS 		// store private key as env var
...
function claimLpReward(_pid=NaN, _amount=0) {
	if (_pid != NaN & _amount != NaN) {
		booLpContract.withdraw(_pid, _amount, options);
		return true;
	} else {
		return false;
	}
...
function depositSingle(_pid=NaN, _amount=0) {
	if (_pid != NaN & _amount != NaN) {
		booSingleContract.deposit(_pid, _amount, options);
		return true;
	} else {
		return false;
	}
...
(async () => {
	const pools = await numPools();
	while(true) {
		// find LPs with claimable balances
		var n; claimablePids = []; claimableBals = [];
		for (n=0; n<pools; n++) {
			await booLpContract.pendingBOO(n, myWallet).then((bal) => {
			    if (bal>0) {
			    	claimablePids.push(n);
			    	// claimableBals.push(bal);
			    	const booBal = ethers.utils.formatUnits(bal,18);
			    	const booBalu256 = ethers.utils.parseUnits(booBal,18);
			    	claimableBals.push(booBalu256);
					console.log("pid"+n+": "+booBal+" BOO");
			    }
				// console.log("pid"+n+": "+bal);
			});
		}
		// console.log(claimablePids);
		// console.log(claimableBals);


		await sleep(1000);

		var claimTotal = await ethers.BigNumber.from(claimableBals[0]);
		for (index in claimablePids) {
			await claimLpReward(claimablePids[index]);		// try withdraw 0 (this is lp token bal)
			await sleep(5000)
			// await claimLpReward(claimablePids[index], claimableBals[index]);
			// var claimBal = await ethers.utils.formatUnits(claimableBals[index],18);
			// claimBal = await parseFloat(claimBal);
			if (index>0) {
				var claimTotal = await claimTotal.add(claimableBals[index]);
			}
		}
		// claimTotal = await String(claimTotal);
		claimTotal = await ethers.utils.formatUnits(claimTotal,18);
		console.log('claimable: ' + claimTotal + ' BOO');

		await sleep(10000);

		// stake single-side
		const tokenBal = await booTokenContract.balanceOf(myWallet);
		// console.log(tokenBal);

		await sleep(5000);

		await depositSingle(0, tokenBal);
		const booDeposit = await ethers.utils.formatUnits(tokenBal,18);
		console.log("deposited " + booDeposit +" BOO");
